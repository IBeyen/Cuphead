...
def get_stats():
    ...
    parry_num_base = module + 0x00268180
    parry_num_offsets = [0x50, 0x5B8, 0x18, 0x30, 0x8, 0xA8, 0x424, 0x0, 0x20]
    actual_address = get_pointer_address(parry_num_base, parry_num_offsets)

    if actual_address:
        # Use read_float if Cheat Engine shows the value type as 'Float'
        # Use read_int if it's '4 Bytes'
        parry_num = pm.read_int(actual_address)
        # print(f"Parry HP: {parry_num}")
    else:
        parry_num = -3000
        # print("Parry not found in memory (pointer chain is broken/null)")
        
    return player_hp, boss_hp, parry_num